...
class FavoriteDetailVM(application: Application) : AndroidViewModel(application) {
    ...
    fun add(favoriteModel: FavoriteModel){
        ...
        if(result > 0){
            Toast.makeText(context, "${favoriteModel.title} " + context.getString(R.string.added_to_favorites), Toast.LENGTH_LONG)
                .show()
        } else {
    ...
    fun delete(favoriteModel: FavoriteModel){
        ...
        if (result > 0){
            Toast.makeText(context, "${favoriteModel.title} " + context.getString(R.string.removed_from_favorites), Toast.LENGTH_LONG)
                .show()
        } else {
            val resultTwo = favoritesHelper.delete(favoriteModel.title+"")
            if (resultTwo > 0){
                Toast.makeText(getApplication(), "${favoriteModel.title} " + context.getString(R.string.removed_from_favorites), Toast.LENGTH_LONG)
                    .show()
            } else {
